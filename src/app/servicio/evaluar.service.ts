 ...
 constructor(public http: HttpClient) {}
  ...
  //  let token = new tokens();
    ...
    //httpImage.headers.append('Authorization','Bearer ' + token.ObtenerToken());
      return this.http.post<EPCCategoriaActividad[]>(this.baseUrl + 'evaluar.php?url=selectbyId',JSON.stringify(categoria),httpOptions);
    }


    selectPuntaje(categoria: CategoriaActividad): Observable<CategoriaActividad[]> {

          return this.http.post<CategoriaActividad[]>(this.baseUrl + 'evaluar.php?url=selectbyPuntaje',JSON.stringify(categoria),httpOptions);
        }
  
    update(categoria: CategoriaActividad): Observable<CategoriaActividad[]> {
          return this.http.post<CategoriaActividad[]>(this.baseUrl + 'evaluar.php?url=update',JSON.stringify(categoria),httpOptions);
        }            
     updatePolitica(): Observable<CategoriaActividad[]> {
      return this.http.post<CategoriaActividad[]>(this.baseUrl + 'evaluar.php?url=updatePolitica',null,httpOptions);
    }  
}